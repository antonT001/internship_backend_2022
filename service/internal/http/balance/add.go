...
func (u *balance) Add(w http.ResponseWriter, r *http.Request) {
	...
	balanceAddIn, err := validateAdd(bodyBytes)
	if err != nil {
		helpers.HttpResponse(w, models.Out{
			Success: false,
	...
	_, err = u.balanceService.Add(balanceAddIn)
	if err != nil {
		helpers.HttpResponse(w, models.Out{
			Success: false,
			Error:   helpers.StringPointer(c.BALANCE + c.SERVICE_ERROR + err.Error()),
...
func validateAdd(bodyBytes []byte) (*models.BalanceFieldsAdd, error) {
	addIn := models.BalanceFieldsAdd{}
	add := models.BalanceAdd{}
